...
const Dashboard: React.FC<DashboardProps> = ({ setView, astroProfile, setAstroProfile, stats, onCheckIn }) => {
  ...
  const getRankName = (lvl: number) => {
    if (lvl < 5) return "Star Seeker";
    if (lvl < 10) return "Cosmic Stargazer";
    if (lvl < 20) return "Void Voyager";
    return "Celestial Sovereign";
  };
    ...
    <div className="space-y-12 animate-in fade-in duration-1000 pb-20">
      <section className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div className="text-center md:text-left">
          ...
          <p className="text-slate-400 font-medium tracking-wide">Rank: {getRankName(stats.level)} • {stats.streak} Day Streak 🔥</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="bg-gradient-to-br from-indigo-900/40 to-black rounded-3xl p-8 md:p-12 text-white shadow-2xl relative overflow-hidden group border border-indigo-500/20 backdrop-blur-md">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom duration-1000">
           <div className="lg:col-span-2 space-y-6">
              <div className="bg-white/5 border border-white/10 rounded-3xl p-8 md:p-12 backdrop-blur-md">
                ...
                {astroProfile.aspects && astroProfile.aspects.length > 0 && (
                  <div className="pt-8 border-t border-white/10">
                    <h4 className="text-sm font-bold text-indigo-300 uppercase tracking-widest mb-4">Major Aspects</h4>
                    <div className="flex flex-wrap gap-2">
                      {astroProfile.aspects.map((aspect, i) => (
                        <span key={i} className="px-3 py-1.5 bg-indigo-900/30 text-indigo-100 text-[10px] font-bold rounded-lg border border-indigo-500/20">{aspect}</span>
                      ))}
                    </div>
                  </div>
                )}
           ...
           <div className="space-y-6">
              ...
              <div className="bg-indigo-600 rounded-3xl p-8 shadow-2xl shadow-indigo-500/20 text-white text-center">
                 <p className="text-[10px] font-bold uppercase tracking-widest opacity-70 mb-2">Communication Style</p>
                 <h4 className="text-2xl font-serif font-bold italic mb-4">"{astroProfile.communicationLabel || 'The Silent Seeker'}"</h4>
      ...
      {showAstroModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 overflow-y-auto">
          <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-2xl p-8 shadow-2xl animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="bg-black/20 p-6 rounded-2xl border border-white/5 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      type="date" 
                      value={astroProfile.birthDate || ''} 
                      onChange={e => setAstroProfile({...astroProfile, birthDate: e.target.value})} 
                      className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-indigo-600 outline-none" 
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <input 
                      ...
                      value={astroProfile.birthLocation || ''} 
                      onChange={e => setAstroProfile({...astroProfile, birthLocation: e.target.value})} 
                      className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-indigo-600 outline-none" 
                    />
                  </div>
                </div>
